...
(function() {
    ...
    const createSettingsUI = () => {
        ...
        note.innerHTML = '⚠️ Reload page after changing settings<br>🔑 <a href="https://aistudio.google.com/app/apikey" target="_blank" style="color:#4CAF50;">Gemini</a> | <a href="https://openrouter.ai/keys" target="_blank" style="color:#4CAF50;">OpenRouter</a> | <a href="https://g4f.space" target="_blank" style="color:#4CAF50;">G4F</a><br>🦆 DuckDuckGo AI is FREE!';
    ...
    const generateAISolution = async () => {
        ...
        if (errorInfo.hasError && errorInfo.currentCode) {
            if (errorInfo.errorType === 'compilation_error') {
                prompt = `You are an elite software security auditor and expert ${language} programmer.
...
5. Audit for logic bugs, runtime errors, edge cases
6. Ensure proper error handling and memory management

=== STRICT OUTPUT RULES ===
- Return ONLY pure executable code
- Do NOT add ANY comments (no //, #, /*, --, etc.)
- Do NOT add explanatory text inside the code
- Do NOT add inline documentation
- Output ONLY the raw program code that compiles and runs
...
=== COMPREHENSIVE AUDIT ===
1. Check NULL pointer dereference and array bounds
2. Check for memory leaks and unfreed allocations
3. Verify loop conditions prevent infinite loops
4. Check division by zero possibilities
5. Validate all edge cases (empty, negative, large inputs)
...
\`\`\`${language.toLowerCase()}`;
                ...
                prompt = `You are an elite software security auditor and expert ${language} programmer. The code below produces WRONG OUTPUT.
...
=== COMPREHENSIVE AUDIT ===
1. SYNTAX: Check brackets, semicolons, imports
2. LOGIC: Off-by-one, wrong operators, type mismatches
3. RUNTIME: Null access, uninitialized vars, bounds
4. EDGE CASES: Empty, negative, zero, large inputs
5. ALGORITHM: Verify correctness and efficiency

=== YOUR TASK ===
1. ANALYZE: Compare expected vs actual output
2. TRACE: Walk through code with input step by step
3. IDENTIFY: Find exact line(s) causing wrong result
4. FIX: Correct the logic error completely
...
1. Verify logic handles all edge cases
2. Check loop bounds and conditions
3. Ensure no memory leaks or null access
4. Validate proper variable initialization
5. Confirm correct algorithm implementation
...
- Do NOT add explanatory text inside the code
- Do NOT add inline documentation
- Output ONLY the raw program code
...
${language === 'C' || language === 'C++' || language === 'C++23' ? '3. Include necessary headers (#include <stdio.h>, etc.)' : ''}

=== COMPREHENSIVE AUDIT ===
1. Syntax: Verify all brackets, types, declarations
2. Logic: Check loop bounds, conditionals, operators
3. Memory: No leaks, proper allocation/deallocation
4. Edge cases: Empty, null, negative, zero, large values
5. Performance: Optimal time/space complexity
6. Error handling: Proper boundary checks
...
\`\`\`${language.toLowerCase()}`;
        ...
        prompt = `You are an elite software security auditor and expert ${language} competitive programmer.
...
1. Syntax: Check brackets, semicolons, imports, declarations
2. Logic: Verify conditionals, loops, operators, return values
3. Runtime: Prevent null access, bounds errors, type mismatches
4. Edge cases: Handle empty, negative, zero, large, special inputs
5. Memory: No leaks, proper resource cleanup
6. Performance: Optimal algorithm, no redundant operations
7. Security: No vulnerabilities, proper input validation
